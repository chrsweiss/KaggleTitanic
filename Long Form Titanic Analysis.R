...
train.datacomb <- data.frame(predrf, predgbm, predlda, Survived=mytrain$Survived)

modcomb <- train(Survived~., data=train.datacomb, method="rf")

predcomb <- predict(modcomb, datacomb)

c_rf <- confusionMatrix(predrf, mytrain$Survived)
c_gbm <- confusionMatrix(predgbm, mytrain$Survived)
c_lda <- confusionMatrix(predlda, mytrain$Survived)
c_comb <- confusionMatrix(predcomb, mytrain$Survived)
...
modrf2 <- train(Survived~Age+Sex+Pclass, data=mytrain, method="rf")
predrf2 <- predict(modrf2, mytrain)
c_rf2 <- confusionMatrix(predrf2, mytrain$Survived)
c_rf2$overall[1]

modrf3 <- train(Survived~Age+Sex+Fare, data=mytrain, method="rf")
predrf3 <- predict(modrf3, mytrain)
c_rf3 <- confusionMatrix(predrf3, mytrain$Survived)
c_rf3$overall[1]
##  97.9% !!!   wow!

predrf3_test <- predict(modrf3, mytest)
c_rf3_test <- confusionMatrix(predrf3_test, mytest$Survived)
c_rf3_test

predrf_test <- predict(modrf, mytest)
c_rf_test <- confusionMatrix(predrf_test, mytest$Survived)
c_rf_test
